...
let top=1
  ...
  case $_osname in
  Linux)
    eval $RHEL_CPUPROCESS > /tmp/memprocess.tmp
  ;;
  HP-UX)
   eval $HPUX_CPUPROCESS > /tmp/memprocess.tmp
   ;;
  AIX)
   eval $AIX_CPUPROCESS > /tmp/memprocess.tmp   
  ;;
  *)
    exit
  ;;
...
writesql $_OutputSqlPath/$cpuprocessfilename $check_start_time $top $_pcmd $_pid $_user $_pcpu $_cputime
let top=$top+1
